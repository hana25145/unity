...
{
    [Range(0.05f, 1f)] [SerializeField] private float steeringMultiplier = 0.6f;
    [Range(0.1f, 1f)] [SerializeField] private float maxSpeedMultiplier = 1f;
    [SerializeField] private float slipperyDamping = 0.05f;
    ...
    [SerializeField] private int maskVAxis = 2;

    private readonly HashSet<PlayerBall> affectedPlayers = new();
    private Texture2D maskTexture;
    private MeshFilter maskMeshFilter;

    public void Configure(float steering, float maxSpeed, float damping)
    {
        steeringMultiplier = Mathf.Clamp(steering, 0.05f, 1f);
        maxSpeedMultiplier = Mathf.Clamp(maxSpeed, 0.1f, 1f);
        slipperyDamping = Mathf.Max(0f, damping);
    }

    public void ConfigureMask(Renderer renderer, float threshold = 0.15f, int uAxis = 0, int vAxis = 2)
    {
    ...
    {
        if (IsSoapPixel(player.transform.position))
            ApplyEffect(player);
        else
            RemoveEffect(player);
    }

    private bool IsSoapPixel(Vector3 worldPosition)
    {
